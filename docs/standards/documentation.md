...
This standard defines the lightweight documentation contract for the template and adopting repositories. Durable project knowledge lives under `docs/`; the complete technical implementation lives under the project-defined boundary recorded in `mk.json`; MK lifecycle tooling lives under `.mk/` without becoming a second documentation authority.
...
- `.mk/` contains repository lifecycle tooling/reports, not product source or durable project knowledge.
...
Repository/system boundaries, dependency direction, implementation boundary, lifecycle-tooling boundary, important flows, extension rules, trade-offs, and limitations.

### Coding Standards
Recurring implementation conventions after technologies are selected; they do not redefine framework source topology.

### AI Bootstrap
Minimal entry context, task routing, implementation-boundary discovery, Graphify use, lifecycle-tool use, stop rules, debug/refactor rules, validation, and reporting.
...
### MK Lifecycle Toolkit
Safe retrofit/migration behavior, protected root categories, manual-review conditions, toolkit installation path, start/health/completion/sync usage, technology-neutral validation, and reporting semantics. The contract lives in [lifecycle-scripts.md](../tooling/lifecycle-scripts.md); scripts are implementation of that contract, not a replacement for it.
...
When implementation topology is complex enough to justify a durable map, add a focused architecture document rather than changing the generic template contract.

## Lifecycle Tooling Documentation Rule

`.mk/toolkit/` in the upstream template and `.mk/scripts/` in adopting projects are executable tooling locations. Durable explanation of their intent and rules belongs under `docs/tooling/`. `.mk/reports/` is local diagnostic output and should not become a committed project journal.

Scripts must report uncertainty as `MANUAL` instead of encoding undocumented project assumptions. Changes to migration safety boundaries or ownership rules require corresponding documentation/ADR updates.
...
- the template CI parses root/bootstrap PowerShell and toolkit PowerShell files before accepting changes to lifecycle tooling.
...
- Let framework/project structure and governance evolve from real requirements.
