...
3. Project documentation and decision records explain intent, contracts, boundaries, and state.
...
3. Read focused architecture, module, or decision context only when needed.
...
- If Graphify is unavailable during a focused task, do not perform an unlimited repository scan. Use the smallest practical source-search fallback, report the limitation, and restore the structural index when practical.
